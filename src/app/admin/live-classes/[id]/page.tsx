  ...
  useEffect(() => {
    async function loadClassData() {
      try {
        ...
        // 1. Get authenticated admin profile
        ...
        const adminUser = {
          ...
          name: `${profile?.first_name || ""} ${profile?.last_name || ""}`.trim() || profile?.email?.split("@")[0] || "Administrator",
        ...
        if (classErr || !classRecord) {
          ...
          setClassDetails({
            ...
            trainerName: classRecord.trainer_name || adminUser.name,
        ...
        setError("Failed to initialize admin classroom.");
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="h-screen w-full bg-[#0B0F19] flex flex-col items-center justify-center gap-3 text-white">
        ...
        <p className="text-xs font-semibold text-zinc-400">Entering Live Classroom as Administrator...</p>
      </div>
    );
  }
